 ...
 *
 * This lets `npm run dev` serve the data produced by `npm run pipeline:deliver:local`.
 * It is a no-op when:
 ...
 * `pipeline:deliver:local`) fails loudly instead of silently returning `index.html`.
 *
...
export function serveTransitData(opts: { basePath: string; origin: string }): Plugin {
  ...
  return {
    ...
    configureServer(server) {
      ...
      // surfacing a wrong VITE_TRANSIT_DATA_PATH or a missing pipeline:deliver:local.
      if (!isExistingDirectory(serveDir)) {
        log.warn(
          `[serve-transit-data] transit data not found: ${path.relative(REPO_ROOT, serveDir)} ` +
            ...
            `Did you run "npm run pipeline:deliver:local"?`,
        );
      }
