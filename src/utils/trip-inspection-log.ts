 ...
 * @returns Raw minutes or a combined inspection string.
 */
export function formatInspectionMinutes(minutes: number, serviceDate?: Date): string {
  if (!serviceDate) {
...
/**
 * Format service-day minutes as `HH:mm` for trip inspection output.
 *
 * @param minutes - Minutes from midnight of the GTFS service day.
 * @param serviceDate - Service day used to resolve overnight times.
 * @returns `HH:mm` or raw minutes when serviceDate is unavailable.
 */
export function formatInspectionClock(minutes: number, serviceDate?: Date): string {
  if (!serviceDate) {
    return String(minutes);
  }
...
/**
 * Build the primary trip inspection summary line.
 *
 ...
 */
export function buildTripInspectionSummaryLog(snapshot: SelectedTripSnapshot): string {
...
/**
 * Build the expanded trip stop listing line for trip inspection.
 *
 * @param snapshot - Reconstructed trip snapshot including the selected stop.
...
export function buildTripInspectionStopsLog(snapshot: SelectedTripSnapshot): string {
  ...
  const tripSpan =
    firstStop && lastStop
      ? `${formatInspectionClock(firstStop.timetableEntry.schedule.departureMinutes, snapshot.serviceDate)}->${formatInspectionClock(lastStop.timetableEntry.schedule.arrivalMinutes, snapshot.serviceDate)}(${lastStop.timetableEntry.schedule.arrivalMinutes - firstStop.timetableEntry.schedule.departureMinutes})`
      : '';
  const stops = snapshot.stopTimes
    .map((stop) => {
      ...
      const stopText = `${stopIndex}:${stopName}(${formatInspectionClock(arrivalMinutes, snapshot.serviceDate)}|${formatInspectionClock(departureMinutes, snapshot.serviceDate)})`;
