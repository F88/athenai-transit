...
describe('getEffectiveHeadsign / getHeadsignDisplayNames parity', () => {
  ...
  it('trip only — both return tripHeadsign', () => {
    ...
    expect(getHeadsignDisplayNames(rd, 'stop', LANG, AGENCY_LANG).resolved.name).toBe('中野駅');
  });
  ...
  it('stop overrides trip — both return stopHeadsign', () => {
    ...
    expect(getHeadsignDisplayNames(rd, 'stop', LANG, AGENCY_LANG).resolved.name).toBe(
      '木場ルート(循環)',
    );
  });
  ...
  it('stop empty fallback — both return tripHeadsign', () => {
    ...
    expect(getHeadsignDisplayNames(rd, 'stop', LANG, AGENCY_LANG).resolved.name).toBe('渋谷駅前');
  });
  ...
  it('lang resolution does not break parity (same effective source)', () => {
    ...
    const names = getHeadsignDisplayNames(rd, 'stop', 'en', AGENCY_LANG);
