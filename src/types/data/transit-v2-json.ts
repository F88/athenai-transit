 ...
 *
 * - trips.wheelchair_accessible — all sources: uniform value, per-trip
 ...
 * direction, and ordered stop sequence. Multiple trips share the same
 * pattern when they follow the same stops in the same order.
 *
...
// Only stops that have at least one stop time record (non-null departure_time) are keyed here.
...
export interface TimetableGroupV2Json {
   ...
   * arrival for the same stop time as `d[sid][i]`.
  ...
  /**
   * Service ID -> pickup_type per stop time.
   * Positionally aligned with {@link d}: `pt[sid][i]` is the
   * pickup_type for the same stop time as `d[sid][i]`.
   * Length MUST equal `d[sid].length` for each service ID.
   *
  ...
  /**
   * Service ID -> drop_off_type per stop time.
   * Positionally aligned with {@link d}: `dt[sid][i]` is the
   * drop_off_type for the same stop time as `d[sid][i]`.
   * Length MUST equal `d[sid].length` for each service ID.
   *
...
export interface TripPatternStatsJson {
  /**
   * Number of trips on this pattern per day for this service group.
   * Counted at origin (si=0) stop times — each trip contributes exactly
   * one count. This is the trip count, not the trip pattern count.
   */
  freq: number;
  /**
 ...
 * may have 50 stop times on weekdays but only 10 on Sundays, or
...
export interface StopGeoJson {
  ...
  cn?: Record<
    string,
    {
       ...
       * summed across unique routes (each route contributes the largest
       * `routeFreqs` value seen at any stop in the neighborhood).
       * Counts all `d`-field entries including non-boardable terminal
       * arrivals — this is a measure of how many vehicles operate in the
       * neighborhood, not how many are boardable. For user-facing
       * "trips you can board", filter by `pickupType` / `isTerminal` at
       * the consumer side.
       */
      freq: number;
...
export interface InsightsBundle {
   ...
   * Only patterns with at least one trip in the service group are included.
   * Patterns with no trips (freq=0) are omitted from the inner record.
   */
   ...
   * Only stops with at least one stop time in the service group are included.
   * Stops with no stop times (freq=0) are omitted from the inner record.
   */
