...
export function StopHistory({ history, selectedStopId, infoLevel, onSelect }: StopHistoryProps) {
  ...
  return (
    <div className="pointer-events-auto absolute top-[calc(4rem+env(safe-area-inset-top))] left-1/2 z-1001 -translate-x-1/2">
      ...
      <Select value="" onValueChange={handleValueChange} open={open} onOpenChange={setOpen}>
        <SelectTrigger className="h-8 max-w-[70vw] gap-1.5 rounded-2xl border-none bg-black/60 px-3 text-sm text-white shadow-md backdrop-blur-sm">
          ...
          <span data-slot="select-value" className={selectedEntry ? 'text-white' : 'text-white/80'}>
            {selectedEntry ? (
              <>
        ...
        <SelectContent
          position="popper"
          className="max-h-60 min-w-48 border-none bg-black/80 text-white backdrop-blur-sm"
        >
          {history.map((entry) => {
            ...
            return (
              ...
              >
                <span className="text-base">{routeTypesEmoji(entry.routeTypes)}</span>
