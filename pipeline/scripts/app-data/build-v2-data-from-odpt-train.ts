 ...
 *
 * Usage:
 *   npx tsx pipeline/scripts/app-data/build-v2-data-from-odpt-train.ts <source-name>
 *   npx tsx pipeline/scripts/app-data/build-v2-data-from-odpt-train.ts --targets <file>
 *   npx tsx pipeline/scripts/app-data/build-v2-data-from-odpt-train.ts --list
 */
...
import { buildAgencyV2 } from './lib/v2-build-odpt-agency';
import { buildCalendarV2 } from './lib/v2-build-odpt-calendar';
import { buildFeedInfoV2 } from './lib/v2-build-odpt-feed-info';
import { buildRoutesV2 } from './lib/v2-build-odpt-routes';
import { buildStopsV2 } from './lib/v2-build-odpt-stops';
import { buildTripPatternsAndTimetableFromOdpt } from './lib/v2-build-odpt-timetable';
import { buildTranslationsV2 } from './lib/v2-build-odpt-translations';
import { writeDataBundle } from './lib/v2-bundle-writer';
...
function printUsage(): void {
  console.log(
    'Usage: npx tsx pipeline/scripts/app-data/build-v2-data-from-odpt-train.ts <source-name>',
  );
  console.log(
    '       npx tsx pipeline/scripts/app-data/build-v2-data-from-odpt-train.ts --targets <file>',
  );
  console.log('       npx tsx pipeline/scripts/app-data/build-v2-data-from-odpt-train.ts --list\n');
  console.log('Options:');
...
async function main(): Promise<void> {
  ...
  if (arg.kind === 'targets') {
    ...
    const scriptPath = resolve(import.meta.dirname, 'build-v2-data-from-odpt-train.ts');
