...
function buildSourceJson(source: BuildSource): void {
  ...
  if (existsSync(existingShapesPath)) {
    const stagedShapes = join(stagingDir, 'shapes.json');
    const stagedSize = existsSync(stagedShapes) ? statSync(stagedShapes).size : 0;
    // Preserve external shapes if the GTFS source produced empty shapes ({})
    if (stagedSize <= 2) {
      const shapesContent = readFileSync(existingShapesPath);
      writeFileSync(stagedShapes, shapesContent);
