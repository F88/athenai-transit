...
const MANAGED_FILES = [
  ...
  'calendar.json',
  'timetable.json',
  // shapes.json is intentionally excluded: it may be produced by this
  // script (from GTFS shapes.txt) OR by build-app-data-from-ksj-railway.ts
  // (from MLIT GeoJSON). Including it here would delete KSJ-generated
  // shapes when the GTFS source has no shape data.
  'agency.json',
  'feed-info.json',
  'translations.json',
];
...
function buildSourceJson(source: BuildSource): void {
  ...
  // pipeline scripts (e.g. shapes.json from build-app-data-from-ksj-railway.ts).
  ...
  try {
    ...
    // Only write shapes.json if the GTFS source has shape data.
    // Empty shapes are omitted so that shapes generated by other
    // pipeline scripts (e.g. KSJ railway) are not overwritten.
  ...
  // preserving files written by other pipeline scripts (e.g. shapes.json
  // from build-app-data-from-ksj-railway.ts).
