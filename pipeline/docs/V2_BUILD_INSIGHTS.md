...
現在の pathDist は停留所座標の Haversine 直線距離の合計で、実際の道路距離より短い。`TripPatternJson.sd` (GTFS shape_dist_traveled) があれば、shapes の polyline に沿った正確な経路距離が得られる。
...
3. **sd がある場合の pathDist 算出**: `sd[sd.length - 1]` が総距離。停留所間距離は `sd[i+1] - sd[i]` で正確に算出可能
