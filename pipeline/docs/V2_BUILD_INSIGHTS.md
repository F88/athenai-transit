...
| lib        | `calendar-walk.ts`, `build-service-groups.ts`, `build-trip-pattern-geo.ts`, `build-trip-pattern-stats.ts`, `build-stop-stats.ts`, `bundle-writer.ts`, `geo-utils.ts` |
...
`buildTripPatternStats(patterns, timetable, serviceGroups, calendar): Record<string, Record<string, TripPatternStatsJson>>`
...
- origin stop (si=0) の trip 数を **1 暦日単位の最大値** として算出する (Issue #219)。 calendar.services + calendar_dates から各日に active な service_id 集合を導き、service group に属する service の trip 数 (= si=0 における d[service_id].length) を日ごとに合計し、calendar 範囲全体での最大日を `freq` とする。
- 旧実装は service group 内の全 service_id の trip 数を calendar に無関係に合算していた。 同一 group に disjoint date range の service が含まれる場合 (例: calendar_dates のみで定義された季節別 services が conservative inclusion で同一群に取り込まれる場合) 実日付では同時 active にならない trip まで足し合わせて過大評価していたため、 暦不変の sum から **暦上で実際に同時 active な services の peak day** へ意味を変更した。
...
- `freq` は per (pattern, group) で 1 数値。 暦概念 (平日/休日 等) は service group key を通じて間接的に表現される — schema 自体は calendar-agnostic な単一値を保持する
...
`buildStopStats(timetable, patterns, routes, serviceGroups, calendar): Record<string, Record<string, StopStatsJson>>`
...
2. calendar.services + calendar_dates から calendar 範囲全体について **日ごとに active な service_id 集合** を precompute
3. 各 stop × service group について:
    - 全 timetable group (全 (patternId, si)) を走査 — terminal arrival を含む
    - service group 内の service_id で stop time がある group を per-service の stop time 数 (= d[service_id].length) として保持
    - `freq`: 上記 per-service counts を **calendar の各日で active な services 分だけ合算し、 calendar 範囲全体での最大日** を `freq` とする (Issue #219)。 暦無視の合算ではなく実日付ベースの 1 暦日最大値
    - `rc`, `rtc`, `ed`, `ld` は calendar-agnostic で group 内の任意 service で観測された値を保持する (旧実装と同じ)
4. `freq = 0` の stop は出力から除外
...
| `pipeline/src/lib/pipeline/app-data-v2/calendar-walk.ts`            | calendar/calendar_dates 走査ヘルパ (pure function) |
