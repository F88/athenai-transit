...
| `routes.route_long_name`         | `emitted-primary`, `emitted-translations` | `RouteV2Json.l` と `translations.route_long_names` の両方に現れる。                                                                |
...
| `trips.trip_headsign`            | `emitted-primary`, `emitted-translations` | `tripPatterns[*].h` と `translations.trip_headsigns` に現れる。                                                                    |
| `stop_times.stop_headsign`       | `emitted-primary`, `emitted-translations` | `tripPatterns.stops[].sh` として一次データ出力 (Issue #92)。翻訳は `translations.stop_headsigns`。                                 |
...
| `agency.agency_phone`            | `emitted-primary`                         | `extractAgenciesV2()` が SELECT し、`AgencyV2Json.ph` (optional) として出力される。値が空なら省略。                                |
| `agency.agency_email`            | `emitted-primary`                         | `extractAgenciesV2()` が SELECT し、`AgencyV2Json.em` (optional) として出力される。値が空なら省略。                                |
| `agency.cemv_support`            | `emitted-primary`                         | `extractAgenciesV2()` が SELECT し、`AgencyV2Json.cemv` (optional, `0` / `1` / `2`) として出力される。空値や非数値は省略。         |
...
| `trips.trip_short_name`          | `not-emitted`                             | schema にはあるが `extractTripPatternsAndTimetable()` でも V2 型でも未対応。現行 raw GTFS でも一部フェリー source に非空値がある。 |
...
| `agency.agency_phone`    | `done` | `emitted-primary`。`AgencyV2Json.ph` (optional) に出力 (空値は省略)。                                                                                                                   |
| `agency.agency_fare_url` | `done` | `emitted-primary`。`agency[].fu` に出力。                                                                                                                                               |
| `agency.agency_email`    | `done` | `emitted-primary`。`AgencyV2Json.em` (optional) に出力 (空値は省略)。                                                                                                                   |
| `agency.cemv_support`    | `done` | `emitted-primary`。`extractAgenciesV2()` が SELECT し、`AgencyV2Json.cemv` (optional, `0` / `1` / `2`) に出力。空値や非数値は省略 (= `parseInt(..., 10)` が `NaN` を返した場合も含む)。 |
...
| `routes.cemv_support`        | `done` | `not-emitted`。PR #199 (a549d916, 2026-05-10) で schema 追加 + `extractRoutesV2()` の SELECT に組み込み済み。ただし JSON output mapping は意図的に保留 (= `RouteV2Json` 不変)。値が入る source 出現時に別 PR で JSON layer を実装する設計。 |
...
| `trips.trip_short_name`       | `done` | `not-emitted`。現行 raw GTFS でも一部フェリー source に非空値があり、silent loss 候補というより既存値の非出力として扱うべき。                                                                                                                                                            |
...
| `trips.safe_duration_factor`  | `done` | `not-emitted`。PR #199 (a549d916, 2026-05-10) で schema 追加 + `extractTripPatternsAndTimetable()` の SELECT に組み込み済み (`TripRow` 内部型に保持)。ただし JSON output mapping は意図的に保留 (= `TripPatternJson` 不変)。値が入る source 出現時に別 PR で JSON layer を実装する設計。 |
| `trips.safe_duration_offset`  | `done` | `not-emitted`。PR #199 (a549d916, 2026-05-10) で schema 追加 + `extractTripPatternsAndTimetable()` の SELECT に組み込み済み (`TripRow` 内部型に保持)。ただし JSON output mapping は意図的に保留 (= `TripPatternJson` 不変)。値が入る source 出現時に別 PR で JSON layer を実装する設計。 |
...
| `stop_times.stop_headsign`                | `done` | `emitted-primary`, `emitted-translations`。一次データ: `tripPatterns.stops[].sh`、翻訳: `translations.stop_headsigns` (Issue #92)。   |
...
| `stop_times.shape_dist_traveled`          | `done` | `not-emitted`。`TripPatternJson.stops[].sd` は型と docs にあるが、現行 `extractTripPatternsAndTimetable()` は未読で出力もしていない。 |
