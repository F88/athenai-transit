...
# 直接実行
npx tsx pipeline/scripts/download-gtfs.ts --targets pipeline/config/targets/download-gtfs.ts
```
...
- name: Download ODPT JSON data
  ...
  run: npx tsx pipeline/scripts/download-odpt-json.ts --targets pipeline/config/targets/download-odpt-json.ts
```
