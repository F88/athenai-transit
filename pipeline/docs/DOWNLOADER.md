...
パイプラインのダウンロードステップ。リソース定義に基づき、外部 API からデータを取得してローカルに保存する。
...
| スクリプト              | 対象      | 入力                                | 出力                       |
| ----------------------- | --------- | ----------------------------------- | -------------------------- |
| `download-gtfs.ts`      | GTFS ZIP  | `pipeline/resources/gtfs/*.ts`      | `pipeline/data/gtfs/`      |
| `download-odpt-json.ts` | ODPT JSON | `pipeline/resources/odpt-json/*.ts` | `pipeline/data/odpt-json/` |
...
```
Usage: npx tsx pipeline/scripts/download-<format>.ts <source-name>
       npx tsx pipeline/scripts/download-<format>.ts --targets <file>
       npx tsx pipeline/scripts/download-<format>.ts --list
       npx tsx pipeline/scripts/download-<format>.ts --help
```
...
Downloading https://api.odpt.org/api/v4/odpt:Station?odpt:operator=odpt.Operator:Yurikamome
  ...
  Duration: 0.1s
  Archived: pipeline/archives/odpt-json/yurikamome/odpt_Station_20260312-130308.json

Saved to pipeline/data/odpt-json/yurikamome/
...
- name: Download ODPT JSON data
  ...
  run: npx tsx pipeline/scripts/download-odpt-json.ts --targets pipeline/targets/download-odpt-json.ts
```
...
ダウンロードしたファイルはタイムスタンプ付きで `pipeline/archives/` にも保存される。データの履歴を追跡するため。
...
`pipeline/archives/` は gitignored。
