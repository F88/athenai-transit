...
v2 パイプラインが出力したバンドルファイルと global artifact を検証する。
...
| 項目       | 内容                                                                                                                  |
| ---------- | --------------------------------------------------------------------------------------------------------------------- |
| 入力       | `pipeline/workspace/_build/data-v2/{prefix}/` (各ソース) + `global/insights.json` + `global/data-source-catalog.json` |
| 出力       | 検証結果のログ                                                                                                        |
...
| Step | 名称                             | 対象モード       | 目的                                                     |
| ---- | -------------------------------- | ---------------- | -------------------------------------------------------- |
| 1    | Unvalidated directory check      | `--targets` のみ | ターゲット外ディレクトリの検出 (data:deliver:local 保護) |
...
V1 と同じ。`--targets` モードでのみ実行。`data-v2/` 内にターゲットリストに含まれないディレクトリが存在する場合、エラー (exit 2)。`global/` ディレクトリは Step 4 / Step 5 で別途検証するため除外される。
...
### GlobalInsightsBundle (`global/insights.json`)

#### GlobalInsightsBundle Structure

| チェック                   | レベル |
| -------------------------- | ------ |
| `bundle_version` = 3       | error  |
| `kind` = "global-insights" | error  |
| `stopGeo.v` = 1            | error  |
| `stopGeo.data` が record   | error  |

### DataSourceCatalogBundle (`global/data-source-catalog.json`)

#### DataSourceCatalogBundle Structure

| チェック                                       | レベル |
| ---------------------------------------------- | ------ |
| ファイルが存在する                             | error  |
| `bundle_version` = 3                           | error  |
| `kind` = "data-source-catalog"                 | error  |
| `metadata.v` = 1                               | error  |
| `metadata.data.createdAt` が string            | error  |
| `sources.v` = 1                                | error  |
| `sources.data` が record                       | error  |
| `globalInsights.v` = 1                         | error  |
| `globalInsights.data.file.sizeBytes` が number | error  |
| `globalInsights.data.counts.stopGeo` が number | error  |

現在の validator は top-level structure だけを確認し、`sources.data` の business correctness や counts 再計算までは行わない。詳細な summary 内容の検証は builder 側テストの責務とする。
