...
- Infra: app のデータ配信を Vercel Blob に切り替え、fetch パスを `/data-v2` から `/data/v2` へ変更した (`vercel.json` + `.vercelignore` + `VITE_TRANSIT_DATA_PATH`)。`vercel.json` の rewrites は常に filesystem の後に評価される (Next.js の `beforeFiles`/`afterFiles` は vercel.json 非対応) ため、static `public/data-v2/` が存在する `/data-v2` では rewrite が発火しない。static 実体を持たない `/data/v2` を新パスにすることで `/data/v2/:path* -> Blob` の array rewrite が常に効く。あわせて移行検証用の `/data-v3 -> Blob` rewrite は `/data/v2` に統合して削除。redundant になった static `public/data-v2/` は `.vercelignore` で deploy から除外 (git 廃止は本番確認後の follow-up)。SW runtimeCaching の urlPattern を `/data-v2` から `/data/v2` へ、CDN header を `/data/v2` (s-maxage=86400) に更新した (#332)。
- Infra/CI: static `public/data-v2` を git から廃止し、transit データ配信を Vercel Blob に一本化した。git commit 方式のデータ更新 workflow (`update-transit-data.yml`) を retire した (削除前に cron を無効化し、削除を revert しても schedule が再稼働しないようにした)。本番は `/data/v2` を Blob から配信する構成 (本番実データで確認済み) のため、redundant な static `public/data-v2/` (143 ファイル) を git から削除した。あわせて `pipeline:deliver:local` の配備先ベースを `public/` から git 管理外の `__LOCAL_AT_DATA__/` (旧称 `__AT_DATA__`、`__LOCAL_*` 規約に統一) に切り替え、既定を `__LOCAL_AT_DATA__/data-v2/` にした。日次のデータ更新は Blob upload workflow が担う (#332)。
