...
- Data: iyotetsu-bus の GTFS resource を 20260901 版へ更新。
- Data: nishi-tokyo-bus の GTFS resource を 20260901 版へ更新。
- Data: kawasaki-city-bus の GTFS resource を 20260828 版へ更新。
- Data: meimon-taiyo-ferry の GTFS resource を 20260901 版へ更新。
- Data: keio-bus の GTFS resource を 20260901 版へ更新。
- Data: kyoto-bus の GTFS resource を 20260901 版へ更新。
...
- Data: keio-bus の GTFS resource を 20260826 版へ更新。
...
- WebApp/dev: dev server 用の Vite plugin (`vite-plugin-serve-transit-data`) を追加した。`npm run pipeline:deliver:local` で配備したローカル配信領域 (`__LOCAL_AT_DATA__/<name>` 等) を `VITE_TRANSIT_DATA_PATH` の末尾セグメントで選択して配信する。存在しない dataset は SPA fallback せず 404 を返し、起動時に配信設定を INFO ログで出力する。
...
- Pipeline: ローカルのデータ配備を pipeline の責務 (Stage 6) として明確化した。配備スクリプトを `scripts/copy-pipeline-data.ts` から `pipeline/scripts/pipeline/copy-pipeline-data.ts` へ移動し、npm script を `data:sync` から `pipeline:deliver:local` へ改名。配備先ベースは `TRANSIT_DATA_DELIVERY_BASE_DIR` (既定 `public/`、`__LOCAL_AT_DATA__/` に切替可)。cross-boundary import 不可 (`.vercelignore` が `pipeline/` を除外) のため dev plugin 側にも定数を複製。重複していた `scripts/lib/sanitize-dir-name.ts` は削除し pipeline の `file-utils` を使用。
...
- Infra: app のデータ配信を Vercel Blob に切り替え、fetch パスを `/data-v2` から `/data/v2` へ変更した (`vercel.json` + `.vercelignore` + `VITE_TRANSIT_DATA_PATH`)。`vercel.json` の rewrites は常に filesystem の後に評価される (`beforeFiles`/`afterFiles` は非対応) ため、static `public/data-v2/` がある `/data-v2` では発火しない。static 実体を持たない `/data/v2` にすることで array rewrite (`-> Blob`) が常に効く。移行検証用の `/data-v3` rewrite は `/data/v2` に統合・削除。SW runtimeCaching の urlPattern を `/data/v2` へ、CDN header を `/data/v2` (s-maxage=86400) に更新した (#332)。
- Infra/CI: static `public/data-v2` を git から廃止し、transit データ配信を Vercel Blob に一本化した。git commit 方式のデータ更新 workflow (`update-transit-data.yml`) を retire (cron 無効化 → 削除の2段階で、削除を revert しても再稼働しない)。本番 `/data/v2` の Blob 配信を実データで確認済みのため、redundant な static `public/data-v2/` を git から削除。あわせて `pipeline:deliver:local` の配備先ベースを `public/` から git 管理外の `__LOCAL_AT_DATA__/` (旧称 `__AT_DATA__`) に変更し、既定を `__LOCAL_AT_DATA__/data-v2/` にした。日次更新は Blob upload workflow が担う (#332)。
- Contracts: v1 専用の wire-format 型 (`StopJson` / `RouteJson` / `AgencyJson` / `ShapesJson` / `TimetableJson` / `TimetableGroupJson`) を `transit-json.ts` から新設の `transit-v1-json.ts` へ移動した。v2 パイプライン/アプリで未使用のため `@deprecated` 注記付きでコメントアウトして参照用に残す (v1 archive docs と整合)。`transit-json.ts` は v2 と共有する不変セクション (calendar / feed info / translations) のみを保持する (#332)。
...
- TransitDisplay: 対象半径 (coverage radius) を絞ってのりばが 0 件になると、 空状態メッセージの early-return で controls 行ごと非表示になり、 `RadiusToggleButton` も消えて半径を広げ直せなくなる行き止まりを修正した (split-flap / dashboard 両 view)。 early-return を廃して controls 行を常時描画し、 panel を `status.state` 別の独立した `&&` ガード (`no-stops` / `no-service` / `ready`) に分離した。 あわせて空状態メッセージを size 駆動にし (split-flap は新設 `MESSAGE_TEXT_CLASS_BY_SIZE`)、 `RadiusToggleButton` には padding で per-size の確定高さを与えて category filter と高さを揃えた (#310)。
- TransitDisplay: 半径内に当日運行のある stop はあるが、 これ以降の便が無い (深夜 / 終バス後) とき、 board もメッセージも出ず画面が空になるバグを修正した。 判定 `resolveTransitDisplayState` が全日軸の `stopServiceState` を見る一方で board は upcoming entries から構築されるズレが原因。 判定を nearby-stop 一覧と同じ `getFilteredTimetableEntriesState` の per-stop + 集約に揃え (`ready` を廃し `FilteredTimetableEntriesState` 語彙へ)、 `service-ended` (「本日の運行は終了しました」) を区別表示するようにした。 container の board 構築ガードは撤去し、 空判定は `buildBoardsForPolicy` の責務へ一本化した (#311, #312)。
- 時刻表行 (stop-times-item / nearby-stop-flat-view) の React key に **service date** が欠けており、 同一 trip が別 service date で再出現すると衝突して行の欠落・誤再描画が起き得たのを修正した。 共有 `buildStopEventKey` (service date 込み、 `JSON.stringify` で patternId の `__` 曖昧性も回避) を両 view と for-ui board builder で共用。 あわせて story fixture の `createEntry` に呼び出しごと distinct な `tripIndex` を与え、 重複 key 警告を解消した (#313)。
...
- Data-source: 板橋区コミュニティバス りんりんGO (国際興業バス運行) を追加。
...
- TimetableDialog: `useTimetable` の所有を新設の `TimetableContainer` (`src/components/timetable/timetable-container.tsx`) に移し、ダイアログ内の datetime 変更が app 全体の再 render を引き起こさないようにした。
- TimetableDialog: DateTimeNavigation の picker 操作で StopBrowser / NearbyStops 系の無関係な component が render されなくなり、体感速度が劇的に向上した。
- TimetableDialog: 新しい時刻表の描画も即時化し、巨大ツリーの再 render と grid 描画の競合を解消した。
- TimetableDialog: `StopsSummary` 等の debug log noise を解消した。
- TimetableDialog: 外部から `openStopTimetable` / `openRouteHeadsignTimetable` を呼ぶ経路 (map markers / BottomSheet / StopSearchDialog) は app.tsx 内の `handleShow*` callback 経由のまま、内部実装だけ `timetableRef` (`useImperativeHandle` 経由) に切り替えた。
...
- Pipeline: 京都バス (KyotoBus Co., Ltd. / 京都バス株式会社) の GTFS-JP データソースを追加 (prefix `kytbus`, route_type 3 bus)。
    ...
    - `shapes.txt` はヘッダのみ (iyt2 / ntbus と同パターン、 `shape_dist_traveleded` 非標準スペル混入) のため shape build には未登録。
    - `translations.txt` は標準 6 列形式で 6 言語 4,192 行 (`en` / `ja` / `ja-Hrkt` / `ko` / `zh-cn` / `zh-tw`)、 `zh-cn` / `zh-tw` は BCP 47 region tag 表記だが PR #191 の `langKeysEquivalent` で `zh-Hans` / `zh-Hant` に自動マッチするため display への影響なし。
    - Brand color `99211F` (deep red)。
...
- Pipeline: 宇和島運輸 (Uwajima Unyu Co., Ltd. / 宇和島運輸フェリー) の GTFS-JP データソースを追加 (prefix `uwjmfry`, route_type 4 ferry)。豊予海峡を渡る四国↔九州 2 路線 (八幡浜〜別府 / 八幡浜〜臼杵) / 3 stops / 90 trips。`shapes.txt` なし、`route_color=FF0000` 有効値。`translations.txt` は東海汽船と同じ legacy 3 列形式 (6 行 / ja-Hrkt のみ)、auto-conversion 経由で取り込み。`calendar.txt` は data 行 0 件で `calendar_dates.txt` のみで運航日を表現する calendar_dates-only feed (PR #160 既対応)。
- Pipeline: 名門大洋フェリー (Meimon Taiyo Ferry Co., Ltd. / シティライン) の GTFS-JP データソースを追加 (prefix `mtfry`, route_type 4 ferry)。関西〜九州を結ぶ夜行フェリー (大阪南港〜新門司港) / 1 route / 2 stops / 4 trips。`shapes.txt` なし、`route_color=0000FF` 有効値。`translations.txt` は legacy 3 列形式 (4 行 / ja-Hrkt のみ)、auto-conversion 経由で取り込み。
- Pipeline: 斎島汽船 (Itsukishima Kisen Co., Ltd.) の GTFS-JP データソースを追加 (prefix `itkfry`, route_type 4 ferry)。
    - 広島県呉市の離島フェリー 2 路線 (斎島〜久比 / 三角島〜久比) / 4 stops / 22 trips、 agency_url は呉市の交通課ページ (事業者独自サイトなし)。
    - 有効期間 2025-10-01〜2026-09-30。
    - `shapes.txt` なし、 両 route で `route_color=FFFFFF`/`route_text_color=000000` (白背景・黒文字、 polyline は shapes.txt 無いため非表示)。
    - `translations.txt` は **標準 6 列形式** (auto-conversion 不要)、 ja-Hrkt + en の翻訳 14 行付き。
    - source data 内の en は訓令式 (`Itukishima`) を採用するが、 display 名は CKAN 表記の Hepburn 式 (`Itsukishima`) を採用。
- About: 東海汽船・桜島フェリー・奥尻島フェリー・オレンジフェリー・宇和島運輸・名門大洋フェリー・斎島汽船のクレジット・データ情報を追加。
...
- Diagnostics: `?diag=repo-bench` に未計測だった 4 メソッド (`getStopMetaById` / `getStopMetaByIds` / `getTripInspectionTargets` / `getTripSnapshot`) の計測を追加。Map-lookup 系は per-call 0.00ms 近傍の sanity check、`getTripInspectionTargets` / `getTripSnapshot` は実 work として全 nearby stops + 各 targets 結果先頭 3 件の snapshot 生成までを計測。`Dataset:` summary log line (sources / stops / routes / trip patterns) も `getAllSourceMeta` 直後に出力するようにし、benchmark report の Environment table を per-source 集計せず populate できるようになった。
- Type: `SourceMeta.stats` に `tripPatternCount` を追加 (`getAllSourceMeta` は benchmark 専用なので production への影響は無し)。`AthenaiRepositoryV2` (v2 merge) と `MockRepository` の両実装で populate。
- Docs: `WEBAPP-BENCHMARKS.md` に `## Purpose` セクションを追加し、ベンチマークの主目的が「計測と記録」であること(「遅い処理を探す」用途ではない)を明記。Measurement Protocol に Source download summary 項目 (per-source data/insights/shapes ファイルサイズ、network 時間は環境依存のため記録対象外) を追加。2026-05-08 baseline (24 sources / 18,648 stops / 1,551 routes / 4,593 trip patterns / 90.5MB total download) を 5 runs median で記録。`getUpcomingTimetableEntries` が前回 (2026-03-31) から -45〜-56% 改善している一方、`shapes` 段が +108% (48→100ms) に伸びているのを vagfr / tome / actvnav 等 6 週間の新規ソース追加で説明。
...
- i18n key `tripInspection.unknownStop` (en: `(Unknown stop)` / ja: `(情報なし)`) を追加。stop name が解決できない時の UI 表示 fallback として使用。
- `RelativeTime` / `StopTimeTimeInfo` に `align` prop (`'left' | 'center' | 'right'`、default `'right'`) を追加。`TripPager` の現在 trip 時刻表示を `align="center"` で中央寄せに切替。
- `RichStopSummary` (TripInspectionDialog) に optional `onSelect` prop を追加し、`Button` (variant="ghost") でラップ。タップ時に `handleSelectStopRow(selectedPatternStopIndex)` を起動し、trip stops list を選択行までスクロール (= "戻る" affordance)。
...
- `info` セマンティック theme color token を追加 (`--info` / `--info-foreground`、`src/index.css`) (#164)。Material Blue 800 (`#1565c0`、`TimetableBoardabilityFilter` の active pill と同色) を anchor とし、light / dark 両モードで `bg-info` / `text-info` / `text-info-foreground` / `border-info` の Tailwind utility を提供。route 色とは独立した semantic accent で status / navigation indicator に利用 (e.g., `TripPager` の trip-index badge)。dark mode は dark bg 上の視認性確保のため Material Blue 500 系の明るめ tone、foreground は両モードとも白で info-badge 慣例に沿う。
...
- `prepareStopTimetable` / `prepareRouteHeadsignTimetable` の 4 番目の引数を `includeTerminals` → `includeNonBoardable` にリネームし、internal filter を `!isTerminal` から `filterBoardable` (= `!isDropOffOnly`) に置換。これにより info-level 駆動の default filter と TimetableModal の boardability toggle が同一 predicate を共有。
- 上記 rename に伴う **behavior change**: simple / normal info level で `pickup_type === 1` の mid-route entries も filter out されるようになった (= GTFS spec の `pickup_type === 1 = boarding not available` semantics に整合)。`TimetableOmitted.terminal` → `TimetableOmitted.nonBoardable` も併せて改名。
...
- `TripInspectionDialog` の `TripEndpointsSummary` 始発 / 終着 cell をクリック可能化 (#164)。クリック時、trip stops list の対応行へ smooth scroll し、`TripInspectionSummary` の position / journey-time bar が反映する `focusedStopIndex` を同期更新 (smooth scroll 完了前に scroll event が止まるため `handleBodyScroll` に頼らず `handleSelectStopRow` 内で `setFocusedStopIndex` を同期実行)。
- 上記クリック化に伴い `SimpleStopSummary` cells を shadcn `<Button variant="ghost">` でラップ (override: `block h-auto w-full p-0`) し、focus-visible ring + `cursor-pointer` + disabled handling を design system に統一 (#164)。
...
- `LabelCountBadge` を `TripPager` から `TripBasicInfo` に移動。`TripBasicInfo` の左端に最小サイズ (`size="xs"` / `size="sm"`) で配置し、残りの要素 (route emoji / AgencyBadge / RouteBadge / headsign) は内側 flex で中央寄せに。`TripBasicInfo` は `currentIndex` / `totalCount` props で trip-index 情報を受け取る形に整理。
- `TripInspectionDialog` の `TripEndpointsSummary` の grid を `[minmax(0,1fr)_auto_minmax(0,1fr)]` から `[minmax(0,max-content)_auto_minmax(0,max-content)]` + `justify-center` に変更。短文時は track が content 幅まで縮み grid 全体が中央配置に、長文時は max-content cap で truncate が効く。
- `SimpleStopSummary` の outer div に `mx-auto w-fit max-w-full` を追加。短いテキスト時に box が grid track 全幅を占有して border が間延びする問題を解消し、box は content 幅、長文時は parent 幅でキャップして内部 truncate を発火。
- `TripStopPlaceholderRow` (`TripStops`) の grid を `[minmax(0,1fr)_auto]` から `[auto_minmax(0,1fr)]` に反転し、`TripStopMetaInfo` を左側 (route 色つき LabelCountBadge) に配置。`TripStopRow` と layout を統一。
- `TripInspectionDialog` の `DialogHeader` の `borderBottomColor` を `var(--background)` に変更。`border-b-10` の太い下線が背景に溶け込み視認上消える状態に。
- `resolveTripStopDisplay` (TripInspectionDialog 内 helper) のシグネチャを変更し、`unknownStopFallback: string` を caller から渡す形に。`stopId` 変数は実 stop_id のみ保持 (undefined-able) し、表示用 fallback は `name → stop_id → unknownStopFallback` の順で解決。
...
- `TripInspectionDialog` / `TripStops` の origin / terminal lookup を sparse-safe 化。`TripSnapshot.stopTimes` は sorted-but-sparse (repo が pattern entry を `(serviceId, tripIndex)` に bind できない場合 row が drop される) なので、`tripStopTimes[0]` / `tripStopTimes[length-1]` だと欠落時に隣接 stop を origin / terminal として誤って返す。新 helper `getOriginStop` / `getTerminalStop` (`patternPosition.isOrigin` / `isTerminal` で find) に置換。
- 上記 sparse-safe 化により、ゆりかもめ短縮便 (例: 22:47 新橋発 23:17 新豊洲止まり、pattern 終点の豊洲が `stop_times` に無い trip) で `TripEndpointsSummary` の Last セルが新豊洲を終点と誤表示する問題を修正。terminal が reconstructed されない場合は `lastStop === undefined` で `(情報なし)` 表示 + disabled 化。
- `TripInspectionDialog` (`SimpleStopSummary`) の長い stop name (例: "しんじゅくえきにしぐち / Shinjuku Sta. Nishiguchi (West Exit)") が ellipsize されない問題を修正。grid track を `1fr` (= `minmax(auto, 1fr)`) から `minmax(0, 1fr)` に変更し、Button 側に `min-w-0` を追加することで、grid item が content の min-content 以下に縮小可能になり、内部 `truncate` utility が正しく発動する。
- UI 表示の `(unknown-stop)` ハードコード fallback を i18n key `tripInspection.unknownStop` に置換。`resolveTripStopDisplay` (TripInspectionDialog) と `TripStopRow` の stopMeta-missing branch、および `TripStopPlaceholderRow` の `Stop-time unavailable (X/Y)` 表示で適用。debug / verbose 系 (verbose-trip-stop-time, trip-inspection-log, TripInspectionCurrentStop / RowsSummary, React key) は debug 用途の英語 placeholder のまま意図的に維持。
