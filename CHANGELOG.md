...
- `TimetableGridEntry` の terminal marker `着` がハードコード日本語だった問題を修正。`timetable.entry.arriving` i18n key を新設 (ja `"着"` / en `"Arr"`) し、`useTranslation` 経由で解決。国際時刻表の業界標準 (Arr/Dep) に準拠。`timetable.entry.*` namespace に配置し、同居する `terminal` / `origin` / `noPickup` / `noDropOff` (これらは pill-style full-word labels) と区別するため component 側に TSDoc コメントを追加。`flat-departure-item.tsx:107` の同種ハードコード `着` は BottomSheet の tight layout 制約があるため今回の scope 外 (既存 `departure.arriving` 空文字 opt-out pattern を維持)。
