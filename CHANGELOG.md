...
- MapView: 地図の円描画を caller 側から制御するための 2 つの optional props を追加した。 `showDistanceRings?: boolean` (既定 true) は既存の `DISTANCE_BANDS` ベースの距離参照同心円 (= 常時 11 ring 描画) の on/off を切り替える。 `highlightedCircles?: HighlightedCircle[]` は新規 export interface `HighlightedCircle` (`{ center: LatLng; radius: number; color: string }`) の配列を受け取り、 新規内部 component `AdditionalCircles` で fill-tinted Circle (`fillOpacity: 0.12` / `weight: 6` / `opacity: 0.85`) として 1 entry ずつ描画する。 center は caller 指定の固定点で、 `useMap` には依存せず map drag に追従しない (= 「stops fetch 時点の中心」 のような stable な anchor を表現)。 color は required + free-form CSS string、 radius も `DISTANCE_BANDS.max` に縛られず free-form なので、 例えば band 外の 150m radius を caller-chosen color で描画することも可能。 既存 call site (= `app.tsx`) は本 PR では未変更で、 props 未指定時の挙動は完全に従来と同一。 後続 PR で AppLayout / TransitDisplaysContainer 側が viewId に応じて `highlightedCircles` を計算 / 「対応半径だけハイライト + 距離参照は隠す」 等の UX に進める scaffolding (#306)。
...
- TimetableHeadsignFilter (timetable dialog): `activeFilters` に「`stopEventAttributesFilteredEntries` 由来の route-headsign key 集合 (= `presentHeadsignKeys`) から外れた key」 (= stale) が残ったままになる問題を修正した。 既存の `dataIdentity` ベース reset (B 案) は dialog の retarget (mode / stop 切替、 close) のみカバーしており、 同一 dialog 内で `showOriginOnly` / `showBoardableOnly` toggle や date navigation により母集合が縮んで stale key が active のまま残るケースで「触れる pill が居ないのに filter は on で全 entries が drop される」 デッドエンドが起き得た。 #304 で導入した `useReconcileIdSet` で render 中に intersection を取り、 stale key を都度除去する。 既存テスト (state-leak) も実 key 形式 (`getRouteHeadsignKey(...)` 出力) ベースに更新し、 「同一 identity の date 変化で entries に残っていれば保持 / 残らなければ除去」 を二分検証する形に再構成した (#305)。
- StopBrowser Agency Filter: `hiddenAgencyIds` に「現在の `presentAgencies` から消えた `agency_id`」 (= stale) が残り続け、 同じ agency が将来また現れたときに過去の hide 設定が勝手に reactivate される block-list 特有の問題を修正した。 `stopBrowserState` (= `hiddenAgencyIds` を含む shared state) は AppLayout に lift されているため `setHiddenAgencyIds` 経由の reconcile は親 component の state 更新になり、 render-time の `useReconcileIdSet` hook は React の「親 component の state を render 中に更新できない」 制約 (`Cannot update a component (AppLayout) while rendering a different component (StopBrowser)`) に抵触する。 そのため `useEffect` + 純関数 `reconcileIdSet` 直呼びで commit 後に同期する形に分けた。 block-list セマンティクスのため reconcile が 1 paint 遅れても「stale id が hide リストにあって何もマッチしない」 状態は無害で、 UX 上の差は出ない。 `useReconcileIdSet` hook は render-time setter 専用 (= 同一 component の `useState` setter) として運用し、 lifted state には effect 形を選択するガイドラインを stop-browser.tsx の call site にコメントで明文化した (#305)。
