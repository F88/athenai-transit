...
- TransitDisplay: 半径内に当日運行のある stop はあるが、 これ以降の便が無い (深夜 / 終バス後) とき、 board もメッセージも出ず画面が空になるバグを修正した。 判定 `resolveTransitDisplayState` が全日軸の `stopServiceState` を見る一方で board は upcoming entries から構築されるズレが原因。 判定を nearby-stop 一覧と同じ `getFilteredTimetableEntriesState` の per-stop + 集約に揃え (`ready` を廃し `FilteredTimetableEntriesState` 語彙へ)、 `service-ended` (「本日の運行は終了しました」) を区別表示するようにした。 container の board 構築ガードは撤去し、 空判定は `buildBoardsForPolicy` の責務へ一本化した (#311)。
