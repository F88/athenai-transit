...
- `FlatDepartureItem` の absolute time 直後にあった terminal marker `着` ハードコードを解消。専用 i18n key `departure.arrivingAbsolute` を新設し `useTranslation` 経由で解決。ja `"着"` は現状維持、en は意図的に `""` (空文字 opt-out) — 既存 `departure.arriving` と同じ locale-controlled opt-out パターンで、BottomSheet の tight per-row layout に en の 3-char `Arr` がオーバーフローするのを回避。`departure.arriving` (relative time 用、`<RelativeTime>` 経由) とは独立した経路で、2 つの terminal marker スロットを locale 側から独立制御できる。component 側に両 key の使い分けを説明する TSDoc コメントを追加。
