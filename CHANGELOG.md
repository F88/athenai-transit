...
- Infra: app のデータ URL (`/data-v2/*`) を Vercel Blob 配信に切り替えた (`vercel.json`)。static `public/data-v2/` が実体として存在する間は filesystem が rewrite より優先されるため、`rewrites` を object 化し `beforeFiles` に `/data-v2 -> Blob` を置いて上書きする。既存の `/data-v3` と SPA fallback は `afterFiles` へ移動 (挙動不変)。URL は same-origin `/data-v2` のままなので SW runtimeCaching / CDN header は変更不要。static `public/data-v2` の git 廃止は本番確認後の follow-up (#332)。
