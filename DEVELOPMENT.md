...
URL に `?mock-data` パラメータを付けると MockRepository に切り替わる。本番ビルドでも使用可能 (意図的にガードなし)。
...
`?diag=<name>` パラメータで診断ツールを実行できる。通常のアプリ描画後に実行され、結果はブラウザコンソールに出力される。diagnostics モジュールは dynamic import のため、通常アクセス時にはロードされない。
